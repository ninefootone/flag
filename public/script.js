...
// --- END SCROLL HANDLER ---
    ...
    // === DISPLAY MODE SETTINGS (Phase 4) ===
    const dmSettingsBtn   = document.getElementById('dm-settings-btn');
    ...
    if (dmSettingsBtn && dmSettingsOverlay) {
        const STORAGE_KEY = 'whistleDisplaySettings';
        ...
        // Open / close overlay
        dmSettingsBtn.addEventListener('click', () => {
            dmSettingsOverlay.classList.toggle('hidden');
        });
        if (dmSettingsClose) {
            dmSettingsClose.addEventListener('click', () => {
                dmSettingsOverlay.classList.add('hidden');
            });
        }
    ...
    const dmFullscreenBtn  = document.getElementById('dm-fullscreen-btn');
    const dmFullscreenIcon = document.getElementById('dm-fullscreen-icon');

    if (dmFullscreenBtn) {
        dmFullscreenBtn.addEventListener('click', () => {
        ...
        document.addEventListener('fullscreenchange', () => {
            if (!dmFullscreenIcon) return;
            const path = dmFullscreenIcon.querySelector('path');
            if (!path) return;
